...
public class ConnectMe {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread.sleep(5000);
		driver.findElement(By.xpath("//input[@id='event-name']")).sendKeys("meeting57");
		Thread.sleep(3000);
		...
		driver.findElement(By.xpath("(//button[@class='btn btn-link ngb-dp-arrow-btn'])[2]")).click();
		Thread.sleep(2000);
		driver.findElement(By.xpath("(//div[@class='btn-light ng-star-inserted'])[15]")).click();
		Thread.sleep(2000);
		...
		List<WebElement> allEvents = driver.findElements(By.xpath("//h5[@class='op-room-name']//div[@class='op-truncate']"));
		String actualEvent = "meeting56";
		System.out.println(allEvents.size());
		Thread.sleep(2000);
		for(int i=0;i<=allEvents.size()-1;i++) {
			//System.out.println("in for");
			//*[@id="mat-tab-content-0-0"]/div/app-op-event-lists/table/tbody/tr[8]/td[3]/div/div[1]/a
			//WebElement textElement = allEvents.get(i);
			if(allEvents.get(i).getText().equals(actualEvent)) {
				//System.out.println("in if ");
				Thread.sleep(2000);
				WebElement scrolling = allEvents.get(i);
				System.out.println(scrolling.getText());
				Point p =scrolling.getLocation();
				JavascriptExecutor javae=(JavascriptExecutor)driver;
				javae.executeScript("window.scrollBy"+ p);
				Thread.sleep(2000);
				WebElement join = driver.findElement(By.xpath("//a[text()=' Join ']"));
				JavascriptExecutor execute = (JavascriptExecutor)driver;
				execute.executeScript("arguments[0].click();", join);
				Thread.sleep(2000);
			}
		}
	}
